...
@section('page_title'){{ trans('nearby-platform.rewards') }} - {{ config()->get('system.premium_name') }} @stop
    ...
    <div class="content text-dark" style="background-image: url('{{ url('assets/images/backgrounds/triangles.jpg') }}');">
      <div class="content-overlay" style="background-color:rgba(245,249,250,0.9)">
        <div class="container">
          <div class="row">
            <div class="col-md-8">
              <h1 class="mb-0">{{ trans('nearby-platform.rewards') }} ({{ $rewards->total() }})</h1>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="row">
              <div class="col-12 col-md-6">
                ...
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active"><a href="javascript:void(0);"><div>{{ trans('nearby-platform.rewards') }}</div></a></li>
                </ol>
          ...
          <div class="row">
            <div class="col-12">
              ...
              <ul class="nav nav-tabs">
                <li class="nav-item">
                  <a class="nav-link active rounded-0" href="{{ url('dashboard/rewards') }}">{{ trans('nearby-platform.rewards') }}</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link rounded-0 color-dark" href="{{ url('dashboard/rewards/leads') }}">{{ trans('nearby-platform.leads') }}</a>
                </li>
              </ul>
              ...
              <table class="table table-hover table-striped mdl-shadow--2dp mb-0">
                <thead class="thead-dark">
                  <tr>
                    ...
                    <th class="text-center d-none d-lg-table-cell">{{ trans('nearby-platform.check_ins') }}</th>
...
foreach ($rewards as $reward) {
  $sl = \App\Http\Controllers\Core\Secure::array2string(['reward_id' => $reward->id]);

  $favicon = $reward->getFavicon();

  $metrics = \DB::table('metrics')->select(['count as views', 'data->fired_at as dates'])->where('type', \App\Metrics\RewardViewCountMetrics::class)->where('metricable_id', $reward->id)->first();
...
<?php if ($reward->image_file_name != null) { ?>
                      <div class="col d-none d-lg-block" style="max-width: 120px">
                        <img id="preview_image" src="{{ $reward->image->url('1x') }}" class="max-width-100 mdl-shadow--4dp">
                      </div>
<?php } ?>
<?php if ($favicon != null) { ?>
                      <div class="col d-none d-lg-block" style="max-width: 32px">
                        <img src="{{ $favicon }}?{{ $reward->updated_at->timestamp }}" style="width: 24px; height: 24px">
                      </div>
<?php } ?>
                      <div class="col">
                        {{ $reward->title }}
                    ...
                    <td class="align-middle d-none d-lg-table-cell">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $reward->created_at)->diffForHumans() }}</td>
                    ...
                    <td class="text-center align-middle d-none d-lg-table-cell">{{ $reward->number_of_times_checked_in }}</td>
                    <td class="text-center align-middle">{{ $reward->number_of_times_redeemed }}</td>
                    <td class="text-center align-middle">

                      <div class="btn-group btn-group-sm" role="group">
                        <a href="{{ url('dashboard/rewards/edit/' . $sl) }}" class="btn btn-sm btn-primary rounded-0 mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.edit') }}"><i class="mi mode_edit"></i></a> 

                        <a href="javascript:void(0);" data-sl="{{ $sl }}" class="btn btn-sm btn-danger rounded-0 btn-delete-reward mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.delete') }}"><i class="mi delete"></i></a>

                        <a href="{{ url('reward/' . \App\Http\Controllers\Core\Secure::staticHash($reward->id)) }}" class="btn btn-sm btn-info rounded-0 mr-2" target="_blank" data-toggle="tooltip" title="{{ trans('nearby-platform.view') }}"><i class="mi search"></i></a>

                        <script src="https://cdnjs.cloudflare.com/ajax/libs/clipboard.js/1.5.13/clipboard.min.js"></script>
                        <script type='text/javascript'>
                         var clipboard = new Clipboard('.btn');
                        </script>
                        <button class="btn btn-sm btn-info rounded-0 mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.copy_to_clipboard') }}" data-clipboard-text="{{ url('reward/' . \App\Http\Controllers\Core\Secure::staticHash($reward->id)) }}"><i class="fas fa-external-link-alt"></i></button> 
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content rounded-0 mdl-shadow--8dp border border-white">
            ...
            <div class="modal-body">
              <img src="<?php echo \Cache::rememberForever('qr-' . md5($reward->getUrl()), function() use($reward) { return DNS2D::getBarcodePNGPath($reward->getUrl(), 'QRCODE', 10, 10, [0,0,0]); }); ?>" alt="barcode" style="width:100%;">
              <div class="m-0 mt-3 form-group"><input type="text" class="form-control rounded-0" value="{{ $reward->getUrl() }}"></div>
            </div>
...
<script>
  $(function() {
    ...
    $('.btn-delete-reward').on('click', function() {
      ...
      swal({
        ...
        if (result.value) {
          ...
          var jqxhr = $.ajax({
            url: "{{ url('dashboard/rewards/delete') }}",
