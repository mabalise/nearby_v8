...
foreach ($coupons as $coupon) {
  ...
  $metrics = \DB::table('metrics')->select(['count as views', 'data->fired_at as dates'])->where('type', \App\Metrics\CouponViewCountMetrics::class)->where('metricable_id', $coupon->id)->first();
...
<?php } ?>
                    ...
                    <td class="text-center align-middle">
                      ...
                      <div class="btn-group btn-group-sm" role="group">
                        ...
                        <a href="{{ url('coupon/' . \App\Http\Controllers\Core\Secure::staticHash($coupon->id)) }}" class="btn btn-sm btn-info rounded-0 mr-2" target="_blank" data-toggle="tooltip" title="{{ trans('nearby-platform.view') }}"><i class="mi search"></i></a>

                        <script src="https://cdnjs.cloudflare.com/ajax/libs/clipboard.js/1.5.13/clipboard.min.js"></script>
                        <script type='text/javascript'>
                         var clipboard = new Clipboard('.btn');
                        </script>
                        <button class="btn btn-sm btn-info rounded-0 mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.copy_to_clipboard') }}" data-clipboard-text="{{ url('coupon/' . \App\Http\Controllers\Core\Secure::staticHash($coupon->id)) }}"><i class="fas fa-external-link-alt"></i></button> 
