...
@section('page_title'){{ trans('nearby-platform.pages') }} - {{ config()->get('system.premium_name') }} @stop
    ...
    <div class="content text-dark" style="background-image: url('{{ url('assets/images/backgrounds/triangles.jpg') }}');">
      <div class="content-overlay" style="background-color:rgba(245,249,250,0.9)">
        <div class="container">
          <div class="row">
            <div class="col-md-8">
              <h1 class="mb-0">{{ trans('nearby-platform.pages') }} ({{ $pages->total() }})</h1>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="row">
              <div class="col-12 col-md-6">
                ...
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active"><a href="javascript:void(0);"><div>{{ trans('nearby-platform.pages') }}</div></a></li>
                </ol>

              </div>
              <div class="col-12 col-md-6 text-left text-md-right mt-2 mt-md-0">

                <a href="{{ url('dashboard/pages/add') }}" class="btn rounded-0 text-white btn-success"><i class="mi add"></i> {{ trans('nearby-platform.add_page') }}</a>
            ...
            <div class="col-12">
              ...
              <table class="table table-hover table-striped mdl-shadow--2dp mb-0">
                <thead class="thead-dark">
                  <tr>
                    <th>{{ trans('nearby-platform.page') }}</th>
...
foreach ($pages as $page) {
  $sl = \App\Http\Controllers\Core\Secure::array2string(['page_id' => $page->id]);
  ...
  $metrics = \DB::table('metrics')->select(['count as views', 'data->fired_at as dates'])->where('type', \App\Metrics\PagesViewCountMetrics::class)->where('metricable_id', $page->id)->first();
...
<?php if ($page->image_file_name != null) { ?>
                      <div class="col d-none d-lg-block" style="max-width: 120px">
                        <img id="preview_image" src="{{ $page->image->url('1x') }}" class="max-width-100 mdl-shadow--4dp">
                      </div>
<?php } ?>
<?php if ($favicon != null) { ?>
                      <div class="col d-none d-lg-block" style="max-width: 34px">
                        <img src="{{ $favicon }}?{{ $page->updated_at->timestamp }}" style="width: 24px; height: 24px">
                      </div>
<?php } ?>
                      <div class="col">
                        {{ $page->title }}
                    ...
                    <td class="align-middle d-none d-lg-table-cell">{{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $page->created_at)->diffForHumans() }}</td>
                    ...
                    <td class="text-center align-middle">
                      ...
                      <div class="btn-group btn-group-sm" role="group">
                        <a href="{{ url('dashboard/pages/edit/' . $sl) }}" class="btn btn-sm btn-primary rounded-0 mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.edit') }}"><i class="mi mode_edit"></i></a> 
                        ...
                        <a href="{{ url('page/' . \App\Http\Controllers\Core\Secure::staticHash($page->id)) }}" class="btn btn-sm btn-info rounded-0 mr-2" target="_blank" data-toggle="tooltip" title="{{ trans('nearby-platform.view') }}"><i class="mi search"></i></a> 
                        ...
                        <button class="btn btn-sm btn-info rounded-0 mr-2" data-toggle="tooltip" title="{{ trans('nearby-platform.copy_to_clipboard') }}" data-clipboard-text="{{ url('page/' . \App\Http\Controllers\Core\Secure::staticHash($page->id)) }}"><i class="fas fa-external-link-alt"></i></button> 
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content rounded-0 mdl-shadow--8dp border border-white">
            ...
            <div class="modal-body">
              <img src="<?php echo \Cache::rememberForever('qr-' . md5($page->getUrl()), function() use($page) { return DNS2D::getBarcodePNGPath($page->getUrl(), 'QRCODE', 10, 10, [0,0,0]); }); ?>" alt="barcode" style="width:100%;">
              <div class="m-0 mt-3 form-group"><input type="text" class="form-control rounded-0" value="{{ $page->getUrl() }}"></div>
            </div>
    ...
    $('.btn-delete-card').on('click', function() {
      ...
      swal({
        ...
        if (result.value) {
          ...
          var jqxhr = $.ajax({
            url: "{{ url('dashboard/pages/delete') }}",
