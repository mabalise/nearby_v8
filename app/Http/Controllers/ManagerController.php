    ...
    */
    ...
    public function showAddUser()
    {
        if (\Gate::allows('manager')) {
            ...
            $limit_reached = \App\Http\Controllers\Core\Helpers::getUserLimitReached($count, $item);
    ...
    public function postUser(): RedirectResponse
    {
        ...
        $user->pages_limit = request()->get('pages_limit');
        $user->deals_limit = request()->get('deals_limit');
        $user->coupons_limit = request()->get('coupons_limit');
        $user->rewards_limit = request()->get('rewards_limit');
        $user->properties_limit = request()->get('properties_limit');
        $user->business_cards_limit = request()->get('business_cards_limit');
        $user->videos_limit = request()->get('videos_limit');
            ...
            if (! empty($user)) {
                ...
                if (! empty($deals)) {
                    foreach ($deals as $deal) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\Deal::class)->where('metricable_id', $deal->id)->delete();
                    }
                }
                ...
                if (! empty($coupons)) {
                    foreach ($coupons as $coupon) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\Coupon::class)->where('metricable_id', $coupon->id)->delete();
                    }
                }
                ...
                if (! empty($rewards)) {
                    foreach ($rewards as $reward) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\Reward::class)->where('metricable_id', $reward->id)->delete();
                    }
                }
                ...
                if (! empty($properties)) {
                    foreach ($properties as $property) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\Property::class)->where('metricable_id', $coupon->id)->delete();
                    }
                }
                ...
                if (! empty($cards)) {
                    foreach ($cards as $card) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\BusinessCard::class)->where('metricable_id', $card->id)->delete();
                    }
                }
                ...
                if (! empty($pages)) {
                    foreach ($pages as $page) {
                        ...
                        // Metrics
                        $metrics = \DB::table('metrics')->where('metricable_type', \App\Page::class)->where('metricable_id', $page->id)->delete();
                    }
                }
