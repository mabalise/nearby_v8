    ...
    */
    ...
    public function viewReward($reward_hash): View
    {
        ...
        if (! empty($reward)) {
            ...
            */
            ...
            for ($step = 0; $step < $reward->steps; $step++) {
                if (isset($check_ins[$step])) {
                    ...
                    if (! is_numeric($check_in)) {
                        $check_in = substr($check_in, 2, strlen($check_in) - 3);
                    }
                    //if (1420030799 < $check_in && $check_in <= 1451566799) {
                    $steps[$step] = [
                        'step' => $step + 1,
                        'checked' => true,
                        'time' => '<div class="day">'.\Carbon\Carbon::createFromTimestamp($check_in)->formatLocalized('%B %d').' </div> <div class="time">'.\Carbon\Carbon::createFromTimestamp($check_in)->tz(auth()->user()->timezone)->formatLocalized('%I:%M %p').'</div>',
                    ];
                    //}
                } else {
                    $steps[$step] = [
            ...
            if ($check_ins != null) {
                if ($reward->steps <= count($check_ins) + 1) {
                    $readyToRedeem = true;
                }
                $last_check_in = $check_ins[count($check_ins) - 1];
                if (! is_numeric($last_check_in)) {
                    $last_check_in = substr($last_check_in, 2, strlen($last_check_in) - 3);
                }
    ...
    public function metaValues($model, $default_value = null)
    {
        foreach ($this->contentFields($model->language) as $var => $prop) {
            ...
            if (! $excludeDefault && $default_value !== null) {
    ...
    public function showLeads(): View
    {
        ...
        if (! in_array($sortby, $allowed_sort)) {
            $sortby = 'created_at';
        }
        if (! in_array($order, ['desc', 'asc'])) {
            $order = 'asc';
    ...
    public function showAddReward()
    {
        ...
        $limit = auth()->user()->rewards_limit;
        $item = trans('nearby-platform.rewards');

        $limit_reached = \App\Http\Controllers\Core\Helpers::getLimitReached($count, $item, $limit);
    ...
    public function postDeleteReward(Request $request): JsonResponse
    {
        ...
        if ($sl != '') {
            ...
            // Delete metrics
            $metrics = \DB::table('metrics')->where('type', \App\Metrics\RewardViewCountMetrics::class)->where('metricable_id', $qs['reward_id'])->delete();
