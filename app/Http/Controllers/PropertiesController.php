    ...
    */
    ...
    public function viewProperty($property_hash): View
    {
        ...
        if (! empty($property)) {
            ...
            if (is_numeric($property->price_sale)) {
                $property->price_sale = $moneyFormatter->format(new \Money\Money($property->price_sale * 100, new \Money\Currency($property->currency_code)));
            }
            if (is_numeric($property->price_rent)) {
                $property->price_rent = $moneyFormatter->format(new \Money\Money($property->price_rent * 100, new \Money\Currency($property->currency_code)));
            }
    ...
    public function showAddProperty()
    {
        ...
        $limit = auth()->user()->properties_limit;
    ...
    public function postDeleteProperty(Request $request): JsonResponse
    {
        ...
        if ($sl != '') {
            ...
            // Delete metrics
            $metrics = \DB::table('metrics')->where('type', \App\Metrics\PageViewCountMetrics::class)->where('metricable_id', $qs['property_id'])->delete();
