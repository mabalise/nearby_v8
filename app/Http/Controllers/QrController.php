...
class QrController extends Controller
{
    ...
    */
    ...
    public function showQrCodes()
    {
        ...
        if (! in_array($sortby, ['updated_at', 'created_at', 'title'])) {
            $sortby = 'updated_at';
        }
    ...
    public function showAddQrCode()
    {
        ...
        $limit_reached = \App\Http\Controllers\Core\Helpers::getQRLimitReached($count, $item);
    ...
    public function postDeleteQrCode(Request $request): JsonResponse
    {
        ...
        if ($sl != '') {
            ...
            // Delete metrics
            $metrics = \DB::table('metrics')->where('type', \App\Metrics\QrViewCountMetrics::class)->where('metricable_id', $qs['qr_code_id'])->delete();
    ...
    public function getDownloadQrCode($sl)
    {
        ...
        if ($sl != '') {
            ...
            $qr_code = \App\Qr::where('user_id', auth()->user()->id)->where('id', $qs['qr_code_id'])->first();
            if (! empty($qr_code)) {
                return response()->download(public_path('attachments/'.$qr_code->qr->path()), 'qr-'.Str::slug($qr_code->title).'.png');
            }
